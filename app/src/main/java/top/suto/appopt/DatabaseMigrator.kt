...
object DatabaseMigrator {
    ...
    fun migrateIfNeeded(context: Context, pkg: String) {
        ...
        fun flush() {
            if (inSession && epoch > 0 && epoch !in existingEpochs) {
                val sessionId = db.insertSession(pkg, epoch, rounds)
                for (line in threadLines) {
                    parseThreadLine(line)?.let { (name, avg, max, series) ->
                        db.insertThread(sessionId, name, avg, max, series)
                    }
                }
                existingEpochs.add(epoch)  // 防止同一文件内重复
                imported++
            }
            threadLines.clear()
        }
     ...
     */
    private fun parseThreadLine(line: String): Quadruple<String, Float, Float, String>? {
        ...
        return Quadruple(name, avg, max, seriesStr)
    }
